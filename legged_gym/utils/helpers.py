...
def update_cfg_from_args(env_cfg, cfg_train, args):
    ...
    if env_cfg is not None:
        ...
        if args.coeff_y_rew is not None:
            env_cfg.rewards.scales.tracking_y_vel = args.coeff_y_rew
        if args.mesh_terrain:
    ...
    if cfg_train is not None:
        ...
        if args.lerp is not None:
            cfg_train.runner.amp_task_reward_lerp = args.lerp

    return env_cfg, cfg_train

def get_args():
    custom_parameters = [
        {"name": "--task", "type": str, "default": "g1_16dof_loco", "help": "Resume training or start testing from a checkpoint. Overrides config file if provided."},
